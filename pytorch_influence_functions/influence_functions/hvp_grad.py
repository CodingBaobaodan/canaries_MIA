...
def s_test_cg(x_test, y_test, model, train_loader, damp, gpu=-1, verbose=True, loss_func="cross_entropy"):
    ...
    print(f"shape of the parameters v_flat = {v_flat.shape}")
    ...
    print(f"shape of the result from cg {result.shape}")
    print(f"first 10 elements of the result {result[:10]}")
...
def s_test(x_test, y_test, model, i, samples_loader, gpu=-1, damp=0.01, scale=25.0, loss_func="cross_entropy"):
    ...
    Arguments:
        ...
        i: the sample number
        samples_loader: torch DataLoader, can load the training dataset. It is 
            the hessian_loader passed from s_test_sample(), which contains random sampled 
            train_loader.dataset with size == recursion depth (the t value)
    ...
    # define H^-1v == v, here, x_test is the Xmal or the ztest from the Influence Function formula 
...
def grad_z(x, y, model, gpu=-1, loss_func="cross_entropy"):
	...
	#print(x.require_grad, y.require_grad)
...
def s_test_sample(
    ...
    Arguments:
        model: pytorch model, for which s_test should be calculated (one model correspond to one s_test)
        x_test: test image
    ...
    for i in range(r):
        ...
        #append the cur_estimate of hvp to the old one 
        with torch.no_grad():
            inverse_hvp = [
                old + (cur / scale) for old, cur in zip(inverse_hvp, cur_estimate)
            ]

    #averaging the result for inverse_hvp
    with torch.no_grad():
