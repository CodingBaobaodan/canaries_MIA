...
def calc_influence_single(
	...
	#For each noise (perturbed image), test_loader contains the z_test (in the formula) which is one pair of x_test and y_test
	#The below z_test is the x_test and t_test is the y_test 
	if s_test_vec is None:
		...
		## this is the bottleneck, but no way to improve it now..
	
	# Calculate the influence function for the target influence loader, i.e the target image or the negative sample image
	train_dataset_size = len(target_influence_loader.dataset)
	influences = []
	for i in range(train_dataset_size):
		z, t = target_influence_loader.dataset[i]
		z = target_influence_loader.collate_fn([z])
		t = target_influence_loader.collate_fn([t])
