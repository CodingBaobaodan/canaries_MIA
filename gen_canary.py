...
def main(args):
	...
	checkpoint_name = args.checkpoint_prefix + f'_{args.num_shadow}.pth'
	...
	targetset = dataset(dataset_name=args.dataset) ### need to change this
	tv_dataset = part_pytorch_dataset(targetset.train_data,targetset.train_label,transform=transform_train)
	...
	args.aug_trainset = part_pytorch_dataset(targetset.train_data,targetset.train_label,transform=transform_aug)
	args.aug_testset = part_pytorch_dataset(targetset.test_data,targetset.test_label,transform=transform_aug)
	...
	#print (type(some_stats.values()))
	return list(some_stats.values())
...
if __name__ == '__main__':
	...
	for this_seed in seed_list:
		args.seed = this_seed
		args.save_name = args.checkpoint_prefix.split('/')[-1]
		#print (args.save_name)
		all_result.append(main(args))
		print (all_result[-1])
		
	all_result = np.array(all_result)
	#print (all_result.shape)
	print (all_result)
	all_result = np.average(all_result,axis=0)
	#print (all_result.shape)
	print (f"AUC {all_result[0]}, TPR @ 0.001 FPR {all_result[2]}")
