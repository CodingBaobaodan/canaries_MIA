...
def cal_stats(final_preds, true_labels):
	...
	auc = metrics.auc(fpr, tpr)
	acc = np.max(1 - (fpr + (1 - tpr)) / 2)
	low = tpr[np.where(fpr < .001)[0][-1]]					 # calculate the TPR at 0.001FPR 
	thresholds_low = thresholds[np.where(fpr < .001)[0][-1]] # calculate the threshold at 0.001FPR 
...
def cal_results(shadow_scores, shadow_in_out_labels, target_scores, target_in_out_labels, in_weight=None, out_weight=None, logits_mul=1,  logits_strategy=None):
	...
	# High final_preds with a True label in true_labels or low final_preds with a False label in true_labels are desired
	...
	some_stats['fix_TPR@0.001FPR'] = low
	...
	some_stats['fix_off_TPR@0.001FPR'] = low
	
	return some_stats

def cal_results_jilin(shadow_scores, shadow_in_out_labels, target_scores, target_in_out_labels, logits_mul=1, logits_strategy=None):
	some_stats = {}
	...
	some_stats['fix_acc'] = acc
	some_stats['fix_TPR@0.001FPR'] = low
	some_stats['fix_threshold@0.001FPR'] = thresholds_low
	some_stats['fix_final_preds'] = final_preds

	# for checking all fpr and tpr
	some_stats['fix_TPR_fpr'] = fpr
	some_stats['fix_TPR_tpr'] = tpr
	
	final_preds, true_labels = lira_offline(shadow_scores, shadow_in_out_labels, target_scores, target_in_out_labels,
											fix_variance=True)
	fpr, tpr, auc, acc, low, _ = cal_stats(logits_mul * final_preds, true_labels)
	some_stats['fix_off_auc'] = auc
	some_stats['fix_off_acc'] = acc
	some_stats['fix_off_TPR@0.001FPR'] = low
...
def split_shadow_models(shadow_models, target_img_id, return_shadow_index=False):
	in_models = []
	out_models = []
	...
	for idx, curr_model in enumerate(shadow_models):
	...
	if (return_shadow_index):
		return in_models, out_models, in_index, out_index
	else:
